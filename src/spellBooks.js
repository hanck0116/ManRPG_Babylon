(function initSpellBooks(global) {
  ...
  function rollCircleByGrade(grade) {
    ...
    if (grade === "마도서") return 7 + Math.floor(Math.random() * 3);
    return 0;
  }
  ...
  function consumeBook(inventory, idx) {
    inventory.spellBooks.splice(idx, 1);
  }
  ...
  function attemptLearnSpellBook(player, inventory) {
    ...
    if (book.grade === "멀티케스팅의 서") {
      player.multiCastingCount = Math.max(1, (player.multiCastingCount || 1) + 1);
      consumeBook(inventory, idx);
    ...
    const wisdom = player.growth.state.effectiveStats?.wisdom || player.growth.state.primaryStats?.wisdom || 1;
    const picked = candidates[Math.floor(Math.random() * candidates.length)];

    if (roll < wisdom) {
      if (!inventory.magicList.find((m) => m.id === picked.id)) inventory.magicList.push({ ...picked });
      consumeBook(inventory, idx);
      return { ok: true, roll, spellName: picked.name, circle };
    }

    if (book.grade === "마도서") consumeBook(inventory, idx);
